...
export const AuthPage: React.FC<AuthPageProps> = ({
  ...
  // Shared panel visibility helper — use opacity+pointerEvents (not display:none) so forms stay in DOM
        ...
        {socialProviders ? (
          socialProviders.map((provider) => (
            <IconButton
              ...
              disabled={loading}
              sx={{ border: 1, borderColor: tokens.colors.border, color: isDark ? tokens.colors.white : tokens.colors.textPrimary, '&:hover': { borderColor: tokens.colors.green, bgcolor: tokens.colors.green, color: tokens.colors.white } }}
            >
              {provider.icon}
            </IconButton>
          ))
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: isDark ? tokens.colors.textPrimary : tokens.colors.background, p: `${tokens.spacing.lg}px` }}>
      ...
      <Box sx={{ position: 'relative', width: '100%', maxWidth: { xs: '100%', md: 1000 }, minHeight: { xs: '100vh', md: 600 }, height: { xs: 'auto', md: 600 }, bgcolor: isDark ? tokens.colors.textSecondary : tokens.colors.surface, borderRadius: { xs: 0, md: `${tokens.radius.xl}px` }, boxShadow: { xs: 'none', md: tokens.shadows.elevated }, overflow: 'hidden', display: 'flex', flexDirection: { xs: 'column', md: 'row' } }}>
        ...
        {/* Gradient panel */}
        <Box sx={{ display: { xs: 'none', md: 'flex' }, position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: `linear-gradient(135deg, ${tokens.colors.green}, ${tokens.colors.earth})`, clipPath: isTransitioning ? 'polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)' : (isSignUp || isForgot) ? 'polygon(0% 0%, 40% 0%, 55% 100%, 0% 100%)' : 'polygon(60% 0%, 100% 0%, 100% 100%, 45% 100%)', transition: 'clip-path 0.8s cubic-bezier(0.65, 0, 0.35, 1)', flexDirection: 'column', alignItems: (isSignUp || isForgot) ? 'flex-start' : 'flex-end', justifyContent: 'center', color: tokens.colors.white, p: `${tokens.spacing.giant}px`, pl: (isSignUp || isForgot) ? `${tokens.spacing.xxxl}px` : `${tokens.spacing.giant}px`, pr: (isSignUp || isForgot) ? `${tokens.spacing.giant}px` : `${tokens.spacing.xxxl}px`, zIndex: 10 }}>
        ...
        <Box sx={{ position: { xs: 'relative', md: 'absolute' }, left: { xs: 'auto', md: 0 }, top: { xs: 'auto', md: 0 }, width: { xs: '100%', md: '50%' }, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', p: formPadding, ...panelSx(currentView === 'signin') }}>
          ...
          <Typography variant="body2" sx={{ mb: `${tokens.spacing.xxxl}px`, color: isDark ? tokens.colors.divider : tokens.colors.textSecondary }}>Welcome back! Please login to your account.</Typography>
          <Box component="form" aria-label="sign-in-form" onSubmit={handleLoginSubmit}>
            ...
            <Box sx={{ textAlign: 'center', mt: `${tokens.spacing.lg}px` }}>
              <Typography variant="body2" color={isDark ? tokens.colors.divider : tokens.colors.textSecondary}>
                ...
                <Link href="#" onClick={(e) => { e.preventDefault(); handleViewChange('signup'); }} fontWeight="bold" sx={{ color: tokens.colors.green, '&:hover': { color: tokens.colors.greenDark } }}>Sign Up</Link>
              </Typography>
            </Box>
        ...
        <Box sx={{ position: { xs: 'relative', md: 'absolute' }, right: { xs: 'auto', md: 0 }, top: { xs: 'auto', md: 0 }, width: { xs: '100%', md: '50%' }, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', p: formPadding, transitionDelay: currentView === 'signup' ? '0.3s' : '0s', ...panelSx(currentView === 'signup') }}>
          ...
          <Typography variant="body2" sx={{ mb: `${tokens.spacing.xxxl}px`, color: isDark ? tokens.colors.divider : tokens.colors.textSecondary }}>Create your account to get started.</Typography>
          <Box component="form" onSubmit={handleSignUpSubmit}>
            ...
            <Box sx={{ textAlign: 'center', mt: `${tokens.spacing.lg}px` }}>
              <Typography variant="body2" color={isDark ? tokens.colors.divider : tokens.colors.textSecondary}>
                ...
                <Link href="#" onClick={(e) => { e.preventDefault(); handleViewChange('signin'); }} fontWeight="bold" sx={{ color: tokens.colors.green, '&:hover': { color: tokens.colors.greenDark } }}>Sign In</Link>
              </Typography>
            </Box>
          </Box>
        </Box>
        ...
        <Box sx={{ position: { xs: 'relative', md: 'absolute' }, right: { xs: 'auto', md: 0 }, top: { xs: 'auto', md: 0 }, width: { xs: '100%', md: '50%' }, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', p: formPadding, transitionDelay: currentView === 'forgot' ? '0.3s' : '0s', ...panelSx(currentView === 'forgot') }}>
          ...
          <Typography variant="body2" sx={{ mb: `${tokens.spacing.xxxl}px`, color: isDark ? tokens.colors.divider : tokens.colors.textSecondary }}>Enter your email and we'll send you a reset link.</Typography>
        ...
        <Box sx={{ position: { xs: 'relative', md: 'absolute' }, left: { xs: 'auto', md: 0 }, top: { xs: 'auto', md: 0 }, width: { xs: '100%', md: '50%' }, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', p: formPadding, ...panelSx(currentView === 'reset') }}>
          ...
          <Typography variant="body2" sx={{ mb: `${tokens.spacing.xxxl}px`, color: isDark ? tokens.colors.divider : tokens.colors.textSecondary }}>Enter your new password below.</Typography>

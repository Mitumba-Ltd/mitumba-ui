...
const sampleImage = 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=800&q=80'
...
function GlassOverlayComponent() {
  ...
  return (
    <Box sx={{ width: '100vw', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#f0f0f0' }}>
      ...
      {showOverlay && (
        <Box sx={{ position: 'absolute', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.1)' }}>
          <MitumbaGlass rounding="large" sx={{ width: '90%', height: '90%', maxWidth: 1000, display: 'flex', flexDirection: 'column', p: 4 }}>
            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 4 }}>
              <MitumbaPrimaryButton label="Back to List" variant="ghost" icon={<ArrowBackIcon />} onClick={() => setShowOverlay(false)} />
            ...
            <Box sx={{ flexGrow: 1, display: 'flex', gap: 4 }}>
              <Box sx={{ width: '60%', borderRadius: 4, overflow: 'hidden' }}>
                <img src={sampleImage} alt="detail" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </Box>
              <Box sx={{ width: '40%', py: 4 }}>
                <Typography variant="h4" sx={{ fontWeight: 900, mb: 2 }}>Nike Airforce1 Premium</Typography>
                <Typography variant="body1" sx={{ color: 'text.secondary', mb: 4 }}>Classic hoops style with durable leather upper and premium cushioning.</Typography>
